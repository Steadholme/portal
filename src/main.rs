//! Portal entry point: init state from env, serve the apex dashboard.
//!
//! Also exposes a dependency-free `portal healthcheck` subcommand used as the container
//! HEALTHCHECK: it GETs `http://127.0.0.1:$PORT/healthz` (port derived from `BIND_ADDR`)
...
async fn main() {
    ...
    let state = match portal::build_state_from_env().await {
        Ok(state) => state,
        Err(e) => {
...
fn run_healthcheck() -> i32 {
    let bind_addr = std::env::var("BIND_ADDR").unwrap_or_else(|_| "127.0.0.1:8600".to_string());
    // Healthcheck always talks to the loopback regardless of the bind interface.
    let port = bind_addr.rsplit(':').next().unwrap_or("8600");
